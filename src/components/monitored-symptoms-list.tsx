				...
				<div>
					<h3
						className={cn("font-medium", {
							"text-green-600":
								symptom.treatment_guidance === TREATMENT_GUIDANCE.NO_CARE,
							"text-amber-600":
...
export default function MonitoredSymptomsList() {
	const { activeSymptoms, resolvedSymptoms, isLoading, resolveSymptom } =
		useMonitoredSymptoms();
	...
	if (isLoading) {
		return (
			<div className="text-center py-8">Loading monitored symptoms...</div>
		);
	}
	...
	if (activeSymptoms.length === 0 && resolvedSymptoms.length === 0) {
		return (
			<div className="text-center py-8 text-gray-500">
				No symptoms currently being monitored
			</div>
		);
	}
